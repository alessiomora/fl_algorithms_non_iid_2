...
class ResBlock(tf.keras.Model):
    def __init__(self, filters, downsample, norm="group", is_first_layer=False, l2_weight_decay=1e-3, seed: Optional[int] = None):
        super().__init__()
        ...
        if not self.is_first_layer:
            self.gn1 = tfa_norms.GroupNormalization(axis=channel_axis, groups=16) if norm == "group" else tf.keras.layers.BatchNormalization(
        ...
        if downsample:
            ...
            self.shortcut = tf.keras.Sequential([
                ...
                tfa_norms.GroupNormalization(axis=channel_axis,
                                             groups=16) if norm == "group" else tf.keras.layers.BatchNormalization(
                    axis=channel_axis, momentum=BATCH_NORM_DECAY, epsilon=BATCH_NORM_EPSILON),
            ])
        else:
            self.conv1 = tf.keras.layers.Conv2D(filters, kernel_size=(3, 3), strides=(1, 1), padding='same',
        ...
        self.gn2 = tfa_norms.GroupNormalization(axis=channel_axis, groups=16) if norm == "group" else tf.keras.layers.BatchNormalization(
            axis=channel_axis, momentum=BATCH_NORM_DECAY, epsilon=BATCH_NORM_EPSILON)
    ...
    def __init__(self, outputs=10, l2_weight_decay=1e-3, seed: Optional[int] = None):
        ...
        self.layer0 = tf.keras.Sequential([
            tf.keras.layers.Conv2D(64, kernel_size=(7, 7), strides=(2, 2), padding='same',
                                   ...
                                   kernel_initializer=tf.keras.initializers.HeNormal(seed=seed),
                                   kernel_regularizer=tf.keras.regularizers.l2(l2_weight_decay)),
            tfa_norms.GroupNormalization(axis=channel_axis, groups=16),
...
class ResNet18LGIC(tf.keras.Model):
    def __init__(self, outputs=10, norm="group", l2_weight_decay=1e-3, seed: Optional[int] = None):
        ...
        self.layer0 = tf.keras.Sequential([
            ...
            tfa_norms.GroupNormalization(axis=channel_axis, groups=16) if norm == "group" else tf.keras.layers.BatchNormalization(
            axis=channel_axis, momentum=BATCH_NORM_DECAY, epsilon=BATCH_NORM_EPSILON),
            tf.keras.layers.ReLU(),
            tf.keras.layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2), padding='same'),
        ], name='layer0')
    ...
    def __init__(self, filters, downsample, is_first_layer=False):
        ...
        if self.is_first_layer:
            ...
            self.conv1 = tf.keras.layers.Conv2D(filters, kernel_size=(3, 3), strides=(1, 1), padding='same',
                                                ...
                                                kernel_initializer=tf.keras.initializers.HeNormal(seed=seed),
                                                kernel_regularizer=tf.keras.regularizers.l2(l2_weight_decay))
            self.gn2 = tfa_norms.GroupNormalization(axis=channel_axis, groups=16)
            self.conv2 = tf.keras.layers.Conv2D(filters, kernel_size=(3, 3), strides=(1, 1), padding='same',
                                                use_bias=False,
                                                kernel_initializer=tf.keras.initializers.HeNormal(seed=seed),
                                                kernel_regularizer=tf.keras.regularizers.l2(l2_weight_decay))
            ...
            if downsample:
                self.gn1 = tfa_norms.GroupNormalization(axis=channel_axis, groups=16)
                self.conv1 = tf.keras.layers.Conv2D(filters, kernel_size=(3, 3), strides=(1, 1), padding='same',
                                                    use_bias=False,
                                                    kernel_initializer=tf.keras.initializers.HeNormal(seed=seed),
                                                    kernel_regularizer=tf.keras.regularizers.l2(l2_weight_decay))
