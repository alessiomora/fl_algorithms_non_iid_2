...
class FedLCModel(tf.keras.Model):

    def __init__(self, model):
        super(FedLCModel, self).__init__()
        self.model = model

    def compile(self, optimizer, loss, metrics, class_occ, tau):
        super(FedLCModel, self).compile(optimizer=optimizer, loss=loss, metrics=metrics)
    ...
    def train_step(self, data):
        ...
        def logit_calibrated_loss(y, logit, class_occ, tau):
            ...
            i_not_equal_y = cal_logit
            # tf.print("\n cal_logit ", cal_logit, summarize=-1)
            denominator = tf.reduce_sum(i_not_equal_y, axis=-1, keepdims=True)
            # tf.print("\n denominator ", denominator, summarize=-1)
            # tf.print("y ", tf.shape(y)) # 100
            # tf.print("cal_logit ", tf.shape(cal_logit)) # 50, 100
            # tf.print("denominator ", tf.shape(denominator)) # 50, 1
            # tf.print("y_logit ", tf.shape(y_logit)) # 50, 1
            calibrated_loss = -tf.math.log(tf.math.divide_no_nan(y_logit, denominator))
            # tf.print(calibrated_loss)
