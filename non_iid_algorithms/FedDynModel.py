...
class FedDynModel(tf.keras.Model):
    """ FedDyn implementation from the paper """
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # tf.print("local_par_list ", tf.shape(local_par_list))
            # tf.print("self.avg_mdl_param ", tf.shape(self.avg_mdl_param))
            # tf.print("self.local_grad_vector ", tf.shape(self.local_grad_vector))
            # loss_dyn = self.alpha * tf.reduce_sum(local_par_list * (-self.local_grad_vector))
            # tf.print("alpha", self.alpha)
            # tf.print("loss_ce", loss_ce)
            # tf.print("loss_dyn", loss_dyn)
            feddyn_loss = loss_ce + loss_dyn
            # tf.print("feddyn_loss", feddyn_loss)
